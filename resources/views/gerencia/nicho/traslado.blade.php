...
<div class="content">
    <div class="page-layout carded left-sidebar">
        ...
        <div class="modal fade" id="devolverDineroModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLabel">¿Seguro que desea liberar el nicho?</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <div class="modal-body">
                        @if($tras_flag >= 0 && $tras_flag < 5)
                            <label>Se hará la devolución del 40%, equivalente a: <strong>S/ {{number_format($contrato->cont_monto*0.4, 2, '.', ',')}}</strong> del precio original. El nicho pasará a estar libre nuevamente.</label>
                        @endif
                        @if($tras_flag >= 5 && $tras_flag < 10)
                            <label>Se hará la devolución del 20%, equivalente a: <strong>S/ {{number_format($contrato->cont_monto*0.2, 2, '.', ',')}}</strong> del precio original. El nicho pasará a estar libre nuevamente.</label>
                        @endif
                        
                        <form method="post" action="/gerencia/contrato/finalizar">
                            <input type="hidden" name="_token" value="{{ csrf_token() }}">
                            <input type="hidden" name="cont_id" value="{{$contrato->cont_id}}">
                        <button type="submit" class="submit-button btn btn-block btn-danger my-4 mx-auto fuse-ripple-ready">LIBERAR</button>
                ...
                <div class="demo-sidebar m-2 ">
                    <ul class="nav flex-column">
                        <li class="subheader">Menú</li>
                            ...
                            @if($tras_flag >= 0 && $tras_flag < 10)
                                <md-divider class="mb-5"> </md-divider>
                                <li class="nav-item">
                                    <button type="button" class="btn btn-block btn-danger ml-5" data-toggle="modal" data-target="#devolverDineroModal" onclick=""> DEVOLVER DINERO <i class="icon-delete"></i></button>
                                </li>
                            @if($tras_flag >= 10)
                                <md-divider class="mb-5"> </md-divider>
                                <li class="nav-item">
